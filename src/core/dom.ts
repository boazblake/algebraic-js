...
export const writeHtml  = (selector: string, html: string) =>
  withElement(selector, el => { if (el) el.innerHTML = html; return el; });

export const appendHtml = (selector: string, html: string) =>
  withElement(selector, el => { if (el) el.insertAdjacentHTML("beforeend", html); return el; });

export const writeText  = (selector: string, text: string) =>
  withElement(selector, el => { if (el) el.textContent = text; return el; });
...
export const alertIO    = (msg: string) => askWindow.map(win => IO(() => win.alert(msg)));
export const scrollToIO = (x: number, y: number) => askWindow.map(win => IO(() => win.scrollTo(x, y)));

export const storageSet = (key: string, val: string) => askStorage.map(s => IO(() => s.setItem(key, val)));
export const storageGet = (key: string) => askStorage.map(s => IO(() => s.getItem(key)));
