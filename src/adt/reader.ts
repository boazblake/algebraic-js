...
Reader.map = <E, A, B>(f: (a: A) => B) => (r: Reader<E, A>): Reader<E, B> => 
  r.map(f);

Reader.chain = <E, A, B>(f: (a: A) => Reader<E, B>) => (r: Reader<E, A>): Reader<E, B> =>
  r.chain(f);

Reader.ap = <E, A, B>(fb: Reader<E, (a: A) => B>) => (fa: Reader<E, A>): Reader<E, B> =>
  fa.ap(fb);

Reader.run = <E, A>(env: E) => (r: Reader<E, A>): A => 
  r.run(env);

/** Local - run with modified environment */
Reader.local = <E, A>(f: (env: E) => E) => (r: Reader<E, A>): Reader<E, A> =>
...
/** Traverse an array */
Reader.traverse = <E, A, B>(f: (a: A) => Reader<E, B>) => (arr: A[]): Reader<E, B[]> =>
