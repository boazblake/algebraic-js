...
Task.tryCatchK = <E, A>(
  ...
  Task(() => f().then(Right).catch((e) => Left(onError(e))));

/** Point-free combinators */
Task.map = <E, A, B>(f: (a: A) => B) => (t: Task<E, A>): Task<E, B> =>
  t.map(f);

Task.chain = <E, A, B>(f: (a: A) => Task<E, B>) => (t: Task<E, A>): Task<E, B> =>
  t.chain(f);

Task.ap = <E, A, B>(fb: Task<E, (a: A) => B>) => (fa: Task<E, A>): Task<E, B> =>
  fa.ap(fb);

Task.mapError = <E, E2>(f: (e: E) => E2) => <A>(t: Task<E, A>): Task<E2, A> =>
  t.mapError(f);
...
Task.getOrElse = <E, A>(defaultValue: A) => (t: Task<E, A>): Promise<A> =>
  t.run().then((ea) => (ea._tag === "Right" ? ea.right : defaultValue));

/** Delay execution */
Task.delay = (ms: number) => <E, A>(t: Task<E, A>): Task<E, A> =>
  Task(() => new Promise((resolve) => setTimeout(() => t.run().then(resolve), ms)));

/** Timeout a task */
Task.timeout = <E>(ms: number, onTimeout: E) => <A>(t: Task<E, A>): Task<E, A> =>
  Task(() =>
    Promise.race([
      t.run(),
...
/** Traverse an array */
Task.traverse = <E, A, B>(f: (a: A) => Task<E, B>) => (arr: A[]): Task<E, B[]> =>
